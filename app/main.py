...
def daily_report(
    ...
    # 2) parse time
    dts = [parse_dt(r["dt"]) for r in rows]
    total_records = len(rows)
    start_time = (rows[0]["tm"] or rows[0]["dt"][-8:])[:5]
    end_time   = (rows[-1]["tm"] or rows[-1]["dt"][-8:])[:5]
    ...
    for i in range(total_records - 1):
        ...
        m, l = int(cur["m"] or 0), int(cur["l"] or 0)
    ...
    total_span_sec = max((dts[-1] - dts[0]).total_seconds(), 1.0)
    utilization_percent = int(round((utilized_sec / total_span_sec) * 100))
